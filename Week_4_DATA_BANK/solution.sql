...
FROM customer_nodes Group By region_id Order By region_id ASC;
...
SELECT 
    ROUND(AVG(DATEDIFF(end_date, start_date)), 0) AS avg_reallocation_days
FROM customer_nodes
...
SELECT 
    region_id,
    MAX(CASE WHEN rn = FLOOR(0.5 * total_count) THEN days END) AS median,
    MAX(CASE WHEN rn = FLOOR(0.8 * total_count) THEN days END) AS p80,
    MAX(CASE WHEN rn = FLOOR(0.95 * total_count) THEN days END) AS p95
FROM CTE
GROUP BY region_id
ORDER BY region_id;
...
SELECT 
	Round(Avg(Count_Txn), 0) As avg_deposit_count,
    Round(Avg(Sum_Txn), 0) As avg_deposit_amount
    FROM CTE1;
...
(SELECT 
	*,
    Last_day(txn_date) As Month_Last_Date,
    CASE WHEN txn_type = "deposit" Then txn_amount*1 ELSE txn_amount*(-1) END As Num_Correction
 FROM customer_transactions),
 CTE2 AS
 (SELECT
	customer_id,
    Month_Last_date,
    Sum(Num_Correction) As Month_Last_Balance
    FROM CTE1 GROUP BY customer_id, Month_Last_date),
CTE3 AS
(SELECT 
	customer_id,
    Month_Last_date,
	Sum(Month_Last_Balance) OVER (
		PARTITION BY customer_id 
        ORDER BY Month_Last_date 
        ROWS BETWEEN unbounded preceding and Current row) As Month_End_Closing_Bal	
	FROM CTE2)
SELECT * FROM CTE3;
...
(SELECT DATE('2020-01-31') AS month_last_date
    UNION ALL
    SELECT LAST_DAY(DATE_ADD(month_last_date, INTERVAL 1 MONTH))
    FROM dates_cte
    WHERE month_last_date < '2020-12-31'),
monthly_txn AS
(SELECT 
        customer_id,
        LAST_DAY(txn_date) AS month_last_date,
        SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END) AS month_total
    FROM customer_transactions
...
(SELECT c.customer_id,
    ...
    CROSS JOIN dates_cte d
    LEFT JOIN monthly_txn m
        ON c.customer_id = m.customer_id
       AND d.month_last_date = m.month_last_date),
closing_balance AS
(SELECT 
        customer_id,
        month_last_date,
        SUM(month_total) OVER (
            PARTITION BY customer_id
            ORDER BY month_last_date
            ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS closing_bal FROM customer_months),
balance_compare AS
(SELECT customer_id,
        closing_bal,
        LAG(closing_bal) OVER (PARTITION BY customer_id ORDER BY month_last_date) AS last_bal
    FROM closing_balance)
SELECT 
    ROUND(COUNT(DISTINCT CASE 
            WHEN last_bal IS NOT NULL
             AND last_bal <> 0
             AND ((closing_bal - last_bal) * 100.0 / last_bal) > 5
            THEN customer_id
        END) * 100.0/ COUNT(DISTINCT customer_id),0) AS percentage_of_customers
FROM balance_compare;
